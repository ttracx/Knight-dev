...
import { openRouterDefaultModelId, openRouterModels, } from "../shared/api";
...
export class OpenRouterHandler {
    ...
    constructor(options) {
        ...
        this.client = new OpenAI({
            ...
            defaultHeaders: {
                "HTTP-Referer": "https://github.com/ttracx/knight-dev", // Optional, for including your app on openrouter.ai rankings.
    ...
    async createMessage(systemPrompt, messages, tools) {
        ...
        const errorMessage = completion.error?.message; // openrouter returns an error object instead of the openai sdk throwing an error
        if (errorMessage) {
    ...
    */
    async streamCompletion(createParams) {
        ...
        const promptTokens = approximateTokenCount(createParams.messages
            .map((m) => (typeof m.content === "string" ? m.content : JSON.stringify(m.content)))
            .join(" "));
        const completionTokens = approximateTokenCount(textContent + toolCalls.map((toolCall) => toolCall.function.arguments || "").join(" "));
        const completion = {
            created: Date.now(),
