...
const SYSTEM_PROMPT = async () => `You are Knight Dev, a highly skilled software developer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
export class KnightDev {
    ...
    providerRef;
    abort = false;
    constructor(provider, apiConfiguration, customInstructions, alwaysAllowReadOnly, task, images, historyItem) {
    ...
    async addToknightMessages(message) {
        this.knightMessages.push(message);
        await this.saveknightMessages();
    }
    async overwriteknightMessages(newMessages) {
        this.knightMessages = newMessages;
        await this.saveknightMessages();
    }
    async saveknightMessages() {
        try {
            const filePath = path.join(await this.ensureTaskDirectoryExists(), "Knight_messages.json");
            ...
            const lastRelevantMessage = this.knightMessages[findLastIndex(this.knightMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"))];
    ...
    async resumeTaskFromHistory() {
        ...
        const lastApiReqStartedIndex = modifiedknightMessages.reduce((lastIndex, m, index) => (m.type === "say" && m.say === "api_req_started" ? index : lastIndex), -1);
        const lastApiReqFinishedIndex = modifiedknightMessages.reduce((lastIndex, m, index) => (m.type === "say" && m.say === "api_req_finished" ? index : lastIndex), -1);
        ...
        const lastRelevantMessageIndex = findLastIndex(modifiedknightMessages, (m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task"));
    ...
    calculateApiCost(inputTokens, outputTokens, cacheCreationInputTokens, cacheReadInputTokens) {
    ...
    async writeToFile(relPath, newContent) {
        ...
        if (newContent === undefined) {
            ...
            await this.say("error", `Knight tried to use write_to_file for '${relPath}' without value for required parameter 'content'. This is likely due to reaching the maximum output token limit. Retrying with suggestion to change response size...`);
            return [
                false,
                await this.formatToolError(`Missing value for required parameter 'content'. This may occur if the file is too large, exceeding output limits. Consider splitting into smaller files or reducing content size. Please retry with all required parameters.`),
            ];
        }
        ...
        try {
            ...
            // await vscode.workspace.applyEdit(edit)
            // Show diff
            await vscode.commands.executeCommand("vscode.diff", vscode.Uri.parse(`Knight-dev-diff:${fileName}`).with({
                query: Buffer.from(originalContent).toString("base64"),
            }), updatedDocument.uri, `${fileName}: ${fileExists ? "Original ↔ Knight's Changes" : "New File"} (Editable)`);
                    ...
                    if (part.added || part.removed) {
                        ...
                        if (activeEditor) {
                            try {
                                ...
                                new vscode.Range(new vscode.Position(startLine, 0), new vscode.Position(Math.min(endLine + 3, activeEditor.document.lineCount - 1), 0)), vscode.TextEditorRevealType.InCenter);
                            }
                            catch (error) {
            ...
            let userResponse;
            if (fileExists) {
                userResponse = await this.ask("tool", JSON.stringify({
    ...
    async closeDiffViews() {
        const tabs = vscode.window.tabGroups.all
            .map((tg) => tg.tabs)
            .flat()
            .filter((tab) => tab.input instanceof vscode.TabInputTextDiff && tab.input?.original?.scheme === "Knight-dev-diff");
        for (const tab of tabs) {
    ...
    async readFile(relPath) {
        ...
        try {
            ...
            await this.say("error", `Error reading file:\n${error.message ?? JSON.stringify(serializeError(error), null, 2)}`);
            return [false, await this.formatToolError(errorString)];
        }
    }
    async listFiles(relDirPath, recursiveRaw) {
        ...
        try {
            ...
            await this.say("error", `Error listing files and directories:\n${error.message ?? JSON.stringify(serializeError(error), null, 2)}`);
            return [false, await this.formatToolError(errorString)];
        }
    }
    ...
    async listCodeDefinitionNames(relDirPath) {
        ...
        try {
            ...
            await this.say("error", `Error parsing source code definitions:\n${error.message ?? JSON.stringify(serializeError(error), null, 2)}`);
            return [false, await this.formatToolError(errorString)];
        }
    }
    async searchFiles(relDirPath, regex, filePattern) {
        ...
        try {
            ...
            await this.say("error", `Error searching files:\n${error.message ?? JSON.stringify(serializeError(error), null, 2)}`);
            return [false, await this.formatToolError(errorString)];
        }
    }
    async executeCommand(command, returnEmptyStringOnSuccess = false) {
        ...
        try {
            ...
            if (userFeedback) {
                ...
                return [
                    true,
                    this.formatToolResponseWithImages(`Command is still running in the user's terminal.${result.length > 0 ? `\nHere's the output so far:\n${result}` : ""}\n\nThe user provided the following feedback:\n<feedback>\n${userFeedback.text}\n</feedback>`, userFeedback.images),
                ];
            }
            ...
            if (completed) {
                ...
                return [
                    false,
                    await this.formatToolResult(`Command is still running in the user's terminal.${result.length > 0 ? `\nHere's the output so far:\n${result}` : ""}\n\nYou will be updated on the terminal status and new output in the future.`),
                ];
            }
        }
        catch (error) {
    ...
    async attemptCompletion(result, command) {
        ...
        return [
            true,
            this.formatToolResponseWithImages(`The user has provided feedback on the results. Consider their input to continue the task, and then attempt completion again.\n<feedback>\n${text}\n</feedback>`, images),
        ];
    }
...
`;
            ...
            const { message, userCredits } = await this.api.createMessage(systemPrompt, this.apiConversationHistory, tools);
    ...
    async recursivelyMakeKnightRequests(userContent, includeFileDetails = false) {
        if (this.abort) {
            throw new Error("KnightDev instance aborted");
        }
        if (this.consecutiveMistakeCount >= 3) {
            const { response, text, images } = await this.ask("mistake_limit_reached", this.api.getModel().id.includes("Knight")
        ...
        try {
            ...
            let cacheCreationInputTokens = response.usage
                .cache_creation_input_tokens || undefined;
            let cacheReadInputTokens = response.usage
            ...
            if (attemptCompletionBlock) {
                let [_, result] = await this.executeTool(attemptCompletionBlock.name, attemptCompletionBlock.input);
                // this.say(
                // 	"tool",
            ...
            if (toolResults.length > 0) {
                if (didEndLoop) {
                    ...
                    const { didEndLoop: recDidEndLoop, inputTokens: recInputTokens, outputTokens: recOutputTokens, } = await this.recursivelyMakeKnightRequests(toolResults);
    ...
    async sayAndCreateMissingParamError(toolName, paramName, relPath) {
        await this.say("error", `Knight tried to use ${toolName}${relPath ? ` for '${relPath}'` : ""} without value for required parameter '${paramName}'. Retrying...`);
        return await this.formatToolError(`Missing value for required parameter '${paramName}'. Please retry with complete response.`);
    }
}
//# sourceMappingURL=KnightDev.js.map