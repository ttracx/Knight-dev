...
export async function getTheme() {
    ...
    try {
        ...
        if (currentTheme === undefined && defaultThemes[colorTheme]) {
            ...
            currentTheme = await fs.readFile(path.join(getExtensionUri().fsPath, "src", "utils", "default-themes", filename), "utf-8");
        }
        // Strip comments from theme
        let parsed = parseThemeString(currentTheme);
        if (parsed.include) {
            const includeThemeString = await fs.readFile(path.join(getExtensionUri().fsPath, "src", "utils", "default-themes", parsed.include), "utf-8");
